...
def add_student() -> None:
    ...
    while True:
        if not re.match(r"\d{4}-d{2}-d{2}",student_dob):
            student_id=input(f"{BCOLORS.FAIL}invalid date of birth, try again:{BCOLORS.endc}")
            continue
        break
...
def add_course()->None:
    ...
    while True:
        for course in COURSES:
            if course["name"]==course_name:
                course_name=input(f"{BCOLORS.FAIL}course already exits, try again:{BCOLORS.ENDC}")
                break
            else:
                break
    ...
    COURSE.append({"name":course_name,"id":course_id})
...
def main():
    ...
    while True:
        ...
        match choice:
            case 1:
                number_of_students_to_add=get_user_input_number("how many students do u wanna add",1)
                for _ in range(number_of_students_to_add):
                    add_student(STUDENTS)
            case 2:
                number_of_students_to_add=get_user_input_number("how many courses do u wanna add",1)
                for _ in range(number_of_students_to_add):
                    add_course(COURSES)
            case 3:
                number_of_students_to_add=get_user_input_number("how many marks do u wanna add",1)
                for _ in range(number_of_students_to_add):
            ...
            case 6:
                for _ in range(5):
                    id =f"22BA-{random.randint(10000,99999)}"
                    while id in[student["id"] for student in STUDENTS]:
                        id=f"22BA-{random.randint(10000,99999)}"
                    name = random.choice(["The","Thi","Thang","Giap","Dieu"])  
                    dob=f"{random.randint(1,31)}/{random.randint(1,12)}/{random.randint(2001,2005)}" 
                ...
                for _ in range(5):
                    ...
                    courses_list=["Maths","Physics","English","Geography"]
